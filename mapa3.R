...
ggplot(brasil_sf) +
  ...
  scale_color_gradient2(
    ...
    name = "Temperature (°C)" # Legenda da escala de cores
  ) +
  geom_text(data = capitals_data,
            aes(x = long, y = lat, label = capital), # Apenas o nome da capital
            vjust = 0, hjust = 0.5, size = 3, check_overlap = TRUE,
            nudge_y = -0.7, # Ajustei o nudge_y um pouco para cima, pois o rótulo é menor
