...
library(ISOweek) # ISOweek2date
library(dlnm) # crossbasis
library(splines) # ns, bs
library(mixmeta) # mixmeta
library(tsModel) # Lag
library(MASS) # mvrnorm
library(dplyr) # Make sure you have dplyr loaded
...
DATE1_CALI15 <- as.Date("2015-01-01")
DATE2_CALI15 <- as.Date("2019-12-26")

# Period for the Calibration: 2004-2008
DATE1_CALI00 <- as.Date("2004-01-01")
DATE2_CALI00 <- as.Date("2008-12-25")

# Period for the Predictions: 2015-2023
DATE1_PRED <- as.Date("2015-01-01")
DATE2_PRED <- as.Date("2023-11-30")

# Period for Year 2023
DATE1_YE23 <- as.Date("2023-01-05")
DATE2_YE23 <- as.Date("2023-11-30")


# For Convenience, We Identify the Date of Each Week with Its Thursday
if(wday(DATE1_CALI00, week_start = 1) != 4 | wday(DATE2_CALI00, week_start = 1) != 4){
  stop("ERROR: Invalid Calibration Period !!!")
}
if(wday(DATE1_CALI15, week_start = 1) != 4 | wday(DATE2_CALI15, week_start = 1) != 4){
  stop("ERROR: Invalid Calibration Period !!!")
}
if(wday(DATE1_PRED, week_start = 1) != 4 | wday(DATE2_PRED, week_start = 1) != 4){
  stop("ERROR: Invalid Prediction Period !!!")
}
if(wday(DATE1_YE23, week_start = 1) != 4 | wday(DATE2_YE23, week_start = 1) != 4){
  stop("ERROR: Invalid Year 2023 Period !!!")
}

#Choose the calibration period
#calibration <- "2015-2019"
...
if(any(0 > PRED_PRC | PRED_PRC > 1)){
  stop("ERROR: Invalid Predicted Temperature Percentiles !!!")
}

# Temperature Percentile Range for the Minimum Mortality Temperature
MIN_PMMT <-   5 / 100 
if(any(0 > MIN_PMMT | MIN_PMMT > 1)){
  stop("ERROR: Invalid Lower Temperature Percentile Range for the Minimum Mortality Temperature !!!")
}
MAX_PMMT <- 100 / 100 
if(any(0 > MAX_PMMT | MAX_PMMT > 1)){
  stop("ERROR: Invalid Upper Temperature Percentile Range for the Minimum Mortality Temperature !!!")
}
...
DATATABLE <- read.csv("TEMPERATURA/PAPERS/DATATABLEBRAZIL.csv")

# Reading the Metatable
#METATABLE <- read.csv("TEMPERATURA/PAPERS/meta.csv")

vREG <- DATATABLE %>%
  distinct(location) %>%
  pull(location) # To get a vector, otherwise it will be a tibble/dataframe
...
#Calibration period 2000-2004 is use to estimate the adaptation
if(calibration == "2015-2019"){
  DATATABLE_CALI <- DATATABLE[which(DATE1_CALI15 <= DATATABLE$date & DATATABLE$date <= DATE2_CALI15 + 7 * MAX_LAG), ]
} else if(calibration == "2004-2008"){
  DATATABLE_CALI <- DATATABLE[which(DATE1_CALI00 <= DATATABLE$date & DATATABLE$date <= DATE2_CALI00 + 7 * MAX_LAG), ]
}
...
if(VAR_FUN == "ns"){
  COEF_MODEL <- matrix(data = NA, nREG, length(VAR_PRC) + 1, dimnames = list(vREG))
}else if(VAR_FUN == "bs"){
  COEF_MODEL <- matrix(data = NA, nREG, length(VAR_PRC) + VAR_DEG, dimnames = list(vREG))
}else{
...
for(iREG in 1:nREG){
  ...
  if(VAR_FUN == "ns"){
    CROSS_BASIS <- crossbasis(DATALIST_CALI[[iREG]]$temp,
                              ...
                              argvar = list(fun = VAR_FUN,
                                            knots = quantile(DATALIST_CALI[[iREG]]$temp, 
                                                             VAR_PRC, 
                                                             na.rm = TRUE),
                                            Boundary.knots = range(DATALIST_CALI[[iREG]]$temp,
                                                                   na.rm = TRUE)),
  ...
  suppressMessages(CROSS_PRED_REG_NOMETA <- crosspred(CROSS_BASIS,
                                                      GLM_MODEL,
                                                      at = quantile(DATALIST_CALI[[iREG]]$temp, 
                                                                    PRED_PRC, 
                                                                    na.rm = TRUE)))
  
  # The Minimum Mortality Temperature Is the Lowest Relative Risk Value within
  #the Predefined Temperature Percentile Range [MIN_PMMT,MAX_PMMT]
...
# Regional Meta-Predictors: Temperature Average (TEMP_AVG), Temperature 
#Inter-Quartile Range (TEMP_IQR)
...
# WARNING: We Exclude the MAX_LAG Weeks that Were Initially Added to Calculate
#All the Attributable Mortality Numbers
vPER_PRED <- as.character(DATALIST_PRED[[1]]$date[1 : (dim(DATALIST_PRED[[1]])[1] - MAX_LAG) ])
nPER_PRED <- length(vPER_PRED)

# Vector of Predicted Time Periods: Individual Weeks, Year 2023
# WARNING: Other Multi-Week Periods Can Be Added Here
vPER_PRED <- c(vPER_PRED, "Year 2023")

# Vector of Temperature Ranges
vRNG <- c("Total", "Total Cold", "Total Heat", "Moderate Cold",
          "Moderate Heat", "Extreme Cold", "Extreme Heat")
nRNG <- length(vRNG)
...
ATT_VAL <- array(NA, dim = c(nREG, length(vPER_PRED), nRNG),
                 dimnames = list(vREG, vPER_PRED, vRNG))
ATT_SIM <- array(NA, dim = c(nREG, length(vPER_PRED), nRNG, nSIM), 
                 dimnames = list(vREG, vPER_PRED, vRNG, 1:nSIM))

# Attributable Number (AN, in Deaths), Fraction (AF, in %) and Incidence 
#(AI, in Deaths per Million) for Regions (REG) and Overall (TOT)
AN_REG = AF_REG = AI_REG = array(NA, dim = c(nREG, length(vPER_PRED), nRNG, 3), 
                                 dimnames = list(vREG  , vPER_PRED, vRNG, c("att_val", "att_low", "att_upp")))
AN_TOT = AF_TOT = AI_TOT = array(NA, dim = c(1 , length(vPER_PRED), nRNG, 3),
                                 dimnames = list("Overall", vPER_PRED, vRNG, c("att_val", "att_low", "att_upp")))

# Mortality (MORT) and Population (POPU) for Regions (REG) and Overall (TOT)
MORT_REG = POPU_REG = array(NA, dim = c(nREG, length(vPER_PRED)), 
                            dimnames = list(vREG  , vPER_PRED))
MORT_TOT = POPU_TOT = array(NA, dim = c(1 , length(vPER_PRED)),
                            dimnames = list("Overall", vPER_PRED))
...
for(iREG in 1:nREG){
  ...
  for(iPER in 1:length(vPER_PRED)){
    ...
    if(iPER  <=   nPER_PRED){
      vTIM <- which(DATALIST_PRED[[iREG]]$date == as.Date(vPER_PRED[iPER]))
    } # Individual Weeks
    else if(vPER_PRED[iPER] == "Year 2023"){
      vTIM <- which(DATE1_YE23 <= DATALIST_PRED[[iREG]]$date & DATALIST_PRED[[iREG]]$date <= DATE2_YE23)
    } # Year 2023
    else{
      stop("ERROR: Invalid Time Period Data !!!")
    }
    if(length(vTIM) <= 0){
      stop("ERROR: Invalid vTIM !!!")
    }
    ...
    for(iRNG in 1:nRNG){
      ...
      else if(vRNG[iRNG] == "Total Cold"){
        vRNG_THRES <- which(DATALIST_PRED[[iREG]]$temp[vTIM] <  MMT_REG[iREG])
      } # Total Cold
      else if(vRNG[iRNG] == "Total Heat"){
        vRNG_THRES <- which(MMT_REG[iREG] <  DATALIST_PRED[[iREG]]$temp[vTIM])
      } # Total Heat
      else if(vRNG[iRNG] == "Moderate Cold"){
        vRNG_THRES <- which(P025[iREG] <= DATALIST_PRED[[iREG]]$temp[vTIM] & DATALIST_PRED[[iREG]]$temp[vTIM] < MMT_REG[iREG])
      } # Moderate Cold
      else if(vRNG[iRNG] == "Moderate Heat"){
        vRNG_THRES <- which(MMT_REG[iREG] <  DATALIST_PRED[[iREG]]$temp[vTIM] & DATALIST_PRED[[iREG]]$temp[vTIM] <= P975[iREG])
      } # Moderate Heat
      else if(vRNG[iRNG] == "Extreme Cold"){
        vRNG_THRES <- which(DATALIST_PRED[[iREG]]$temp[vTIM] < P025[iREG])
      } # Extreme Cold
      else if(vRNG[iRNG] == "Extreme Heat"){
        vRNG_THRES <- which(P975[iREG] <  DATALIST_PRED[[iREG]]$temp[vTIM])
      } # Extreme Heat
      else{
        stop("ERROR: Invalid Temperature Range !!!")
      }
      if(length(vRNG_THRES) > 0 & sum(LAGGED_MORT_VECTOR[vTIM]) > 0){
        ...
        if(!is.finite(ATT_VAL[iREG,iPER,iRNG ])){
          stop("ERROR: Invalid ATT_VAL !!!")
        }
        ...
        ATT_SIM[iREG,iPER,iRNG,] <- colSums(ATT_NUM_TS_SIM[ vTIM[vRNG_THRES], , drop=FALSE ], na.rm = TRUE) *
          sum(rowMeans(LAGGED_MORT_MATRIX[ vTIM, , drop=FALSE ], na.rm = TRUE), na.rm = TRUE) /
          sum(LAGGED_MORT_VECTOR[vTIM], na.rm = TRUE)
        if(any(!is.finite(ATT_SIM[iREG,iPER,iRNG,]))){
          stop("ERROR: Invalid ATT_SIM !!!")
        }
        ...
        if(MORT_REG[iREG,iPER] > 0){
          AF_REG[iREG,iPER,iRNG,] <- 10^2 * AN_REG[iREG,iPER,iRNG,] / MORT_REG[iREG,iPER]
        }
        
        # Attributable Incidence (in Deaths per Million)
        if(POPU_REG[iREG,iPER] > 0){
          AI_REG[iREG,iPER,iRNG,] <- 10^6 * AN_REG[iREG,iPER,iRNG,] / POPU_REG[iREG,iPER]
        }
        else{
          stop("ERROR: Invalid Regional Population !!!")
        }
...
# Heat Related Mortality for Year 2023
# WARNING: Numbers Do Not Need to Coincide with Those in the Manuscript
print("Heat Related Mortality for Year 2023:")
print(round(rbind(AN_REG[ , "Year 2023", "Total Heat", ],
                  AN_TOT[ , "Year 2023", "Total Heat", ]), digits = 0))
...
plot(as.Date(vPER_PRED[1:nPER_PRED]), AN_TOT[1,1:nPER_PRED,"Total Cold","att_val"],
     col = rgb(0.00,0.00,1.00), lwd = 2, lty = 1, type = "l", 
     main = "Temperature Related Mortality", 
     xlab = "Time (week)", 
     ylab = "Mortality (weekly deaths)",
     xaxt = "n")
polygon(c(as.Date(vPER_PRED[1:nPER_PRED]), 
          rev(as.Date(vPER_PRED[1:nPER_PRED]))),
        c(AN_TOT[1,1:nPER_PRED,"Total Cold","att_low"],
          rev(AN_TOT[1,1:nPER_PRED,"Total Cold","att_upp"])),
        col = rgb(0.00,0.00,1.00,1/3),
        border = FALSE)
polygon(c(as.Date(vPER_PRED[1:nPER_PRED]),
          rev(as.Date(vPER_PRED[1:nPER_PRED]))),
        c(AN_TOT[1,1:nPER_PRED,"Total Heat","att_low"], 
          rev(AN_TOT[1,1:nPER_PRED,"Total Heat","att_upp"])), 
        col = rgb(1.00,0.00,0.00,1/3), border = FALSE)
lines(as.Date(vPER_PRED[1:nPER_PRED]),
      AN_TOT[1,1:nPER_PRED,"Total Cold","att_val"],
      col = rgb(0.00,0.00,1.00), lwd = 2, lty = 1)
lines(as.Date(vPER_PRED[1:nPER_PRED]),
      AN_TOT[1,1:nPER_PRED,"Total Heat","att_val"],
      col = rgb(1.00,0.00,0.00), lwd = 2, lty = 1)
abline(col = rgb(0.00,0.00,0.00), lwd = 1, lty = 1, h = 0)
legend("top", c("Total Cold", "Total Heat"), 
       col = rgb(c(0.00,1.00), c(0.00,0.00), c(1.00,0.00)), 
       lwd = 2, lty = 1, box.lty = 0, 
       horiz = TRUE, bg = "transparent")
axis(1, at = as.Date(c("2015-07-01", "2016-07-01", "2017-07-01", "2018-07-01",
...
for(iREG in 1:nREG){
  ...
  lines(CROSS_PRED_REG_META[[iREG]]$predvar, CROSS_PRED_REG_META[[iREG]]$allRRfit,
        col = rgb(0.00,0.00,0.00), lwd = 4, lty = 1)
