...
library(purrr) 
  ...
  # Reordena as colunas
  temperatura <- temperatura[c(1, 6, 7, 8, 4, 5, 2, 3)]
  ...
  temperatura <- temperatura[c(1, 2, 3, 4, 5, 7, 9)]
  ...
  temperatura <- rbind(temperaturaconv, temperatura)

  temperatura <- temperatura %>%
    group_by(DATA, Mes, Ano, DIA_MES,DC_NOME, ESTADO) %>%
    summarise(
  ...
  dados_cidades <- temperatura %>%
    ...
    unique() %>%
    mutate(Codigo_IBGE = NA_character_)
  ...
  temperatura <- temperatura %>%
    mutate(Y34a = ifelse(temperatura_maxima >= 34 & temperatura_maxima < 36, TRUE, FALSE))
  ...
  temperatura <- temperatura %>%
    mutate(
      Intensidade_HW = case_when(
        EHF > (3 * EHF85) ~ "Extrema",
        EHF > EHF85 ~ "Severa",
        EHF > 0 & EHF < EHF85  ~ "Baixa Intensidade",
        TRUE ~ "Não HW"
      )
    )
