...
diretorio_origem <- paste0('TEMPERATURA/TEMPERATURA/AUTOMATICAS/', ANO)
...
for (caminho_arquivo in arquivos_para_mover) {
  ...
  dados_final <- rbind(dados_final, dados)
  
}

nome_arquivo_csv <- paste0("TEMPERATURA/TEMPERATURA/AUTOMATICAS/2007.csv")
write.csv(dados_final, file = nome_arquivo_csv, row.names = FALSE, fileEncoding = "UTF-8")